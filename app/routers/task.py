...
async def get_tasks(
    session: Session = Depends(get_db), user: str = Depends(require_user)
):
    tasks = session.query(Task).filter(Task.user_id == user).all()
...
async def get_task(
    id: str, session: Session = Depends(get_db), user: str = Depends(require_user)
):
    task_model = session.query(Task).filter(Task.id == id).first()
    if not task_model:
        raise HTTPException(
